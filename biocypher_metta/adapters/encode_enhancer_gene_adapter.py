...
class EncodeEnhancerGeneLinkAdapter(Adapter):
    ...
    def __init__(self, filepath, source, source_url, biological_context):
        if source not in EncodeEnhancerGeneLinkAdapter.ALLOWED_SOURCES:
            raise ValueError('Invalid source. Allowed values: ' +
                             ','.join(EncodeEnhancerGeneLinkAdapter.ALLOWED_SOURCES))
        ...
        super(EncodeEnhancerGeneLinkAdapter, self).__init__()
    ...
    def get_edges(self):
        with gzip.open(self.filepath, 'rt') as input_file:
            ...
            for row in reader:
                ...
                class_name = row[4]
                regulatory_element_id = build_regulatory_region_id(chr, start, end)
                score = row[9]

                gene_id = row[6]
                _id = regulatory_element_id + '_' + gene_id + '_' + self.biological_context
                _source = regulatory_element_id
                _target = gene_id
                _props = {
                    'score': score,
                    'source': self.source,
